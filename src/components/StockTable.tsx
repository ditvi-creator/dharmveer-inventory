...
export const MovementInput = ({ currentTotal, onAdd }: { currentTotal: number, onAdd: (val: number) => void }) => {
  ...
  return (
    <div className="flex flex-col items-center">
      <div className="text-[10px] text-gray-400 dark:text-gray-400 font-medium mb-0.5 leading-none">Tot: {currentTotal}</div>
      <input
        type="number"
        value={val}
        ...
        onBlur={commitValue}
        placeholder="+0"
        className="w-16 text-center border border-transparent hover:border-gray-200 dark:border-gray-700 focus:border-blue-500 focus:bg-white dark:bg-gray-800 rounded py-0.5 px-1 text-sm bg-transparent font-bold transition-all outline-none [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
      />
    </div>
  );
};
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-gray-50 dark:bg-gray-900/50 border-b border-gray-200 dark:border-gray-700">
              ...
              <th colSpan={2} className="px-4 py-2 text-center text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider border-b border-r border-gray-100 dark:border-gray-800">Opening Stock</th>
              ...
              <th rowSpan={2} className="px-4 py-4 text-xs font-bold text-gray-600 dark:text-gray-300 uppercase tracking-wider">Party Name</th>
            ...
            <tr className="bg-gray-50 dark:bg-gray-900/50 border-b border-gray-200 dark:border-gray-700">
              <th className="px-4 py-2 text-center text-[15px] font-bold text-gray-400 dark:text-gray-400 uppercase border-r border-gray-100 dark:border-gray-800">MP</th>
              <th className="px-4 py-2 text-center text-[15px] font-bold text-gray-400 dark:text-gray-400 uppercase border-r border-gray-100 dark:border-gray-800">KL</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-100">
            <AnimatePresence>
              {items.length === 0 ? (
                <tr>
                  <td colSpan={13} className="px-4 py-16 text-center text-gray-400 dark:text-gray-400 text-sm italic">
                ...
                items.map((item, index) => {
                  ...
                  return (
                    <motion.tr 
                      ...
                      animate={{ opacity: 1 }}
                      exit={{ opacity: 0 }}
                      className={`hover:bg-gray-50 dark:bg-gray-900/50 transition-colors group ${isLowStock ? 'bg-red-50 dark:bg-red-900/20' : ''}`}
                    >
                      ...
                      <td className="px-4 py-4 text-sm text-gray-600 dark:text-gray-300 border-r border-gray-50 dark:border-gray-800/50">
                        {item.size}
                      </td>
                      <td className="px-4 py-4 text-sm text-gray-500 dark:text-gray-400 border-r border-gray-50 dark:border-gray-800/50 uppercase">
                        {item.unit || 'BOX'}
                      </td>
                      
                      <td className="px-2 py-4 text-sm text-center font-bold border-r border-gray-50 dark:border-gray-800/50 bg-gray-50 dark:bg-gray-900/50">
                        <input
                          type="number"
                          ...
                          placeholder="0"
                          className="w-16 text-center border border-transparent hover:border-gray-200 dark:border-gray-700 focus:border-blue-500 focus:bg-white dark:bg-gray-800 rounded py-1 px-1 text-sm bg-transparent font-bold transition-all outline-none [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                        />
                      </td>
                      ...
                      <td className="px-4 py-4 text-sm text-center text-gray-500 dark:text-gray-400 border-r border-gray-50 dark:border-gray-800/50">
                        {item.reorderLevel}
                      </td>

                      <td className="px-4 py-4 text-sm text-center font-bold border-r border-gray-50 dark:border-gray-800/50">
                        {item.booked}
                      </td>
                            ...
                            {item.bookings && item.bookings.length > 0 ? (
                              <div className="space-y-2">
                                {item.bookings.map((booking) => (
                                  <div key={booking.id} className="text-xs bg-gray-50 dark:bg-gray-900/50 p-2.5 rounded-lg border border-gray-100 dark:border-gray-800">
                                    <div className="flex justify-between items-start gap-2 mb-1">
                                      <div className="flex items-center gap-2">
                                        <span className="font-bold text-gray-800 dark:text-gray-100 break-words text-[15px]">{booking.partyName || 'Unknown'}</span>
                                        <button 
                                          onClick={(e) => {
                                            e.stopPropagation();
                                            onOpenChallan(item, booking);
                                          }}
                                          className="text-gray-400 dark:text-gray-400 hover:text-[#2962d9] transition-colors p-0.5"
                                          title="Print Delivery Challan"
                                        >
                                          <Printer className="w-[20px] h-[20px] bg-white dark:bg-gray-800 rounded-none ml-[10px]" />
                                        </button>
                                      </div>
                                    ...
                                    {booking.address && (
                                      <div className="text-gray-500 dark:text-gray-400 text-[14px] leading-snug break-words">{booking.address}</div>
                                    )}
                                    {booking.dateOfSend && (
                                      <div className="mt-2 pt-2 border-t border-gray-200 dark:border-gray-700/50 flex flex-col gap-1 text-[#6b7280] text-[12px]">
                                        ...
                                        <div className="flex items-center justify-between pl-5">
                                          ...
                                          <button 
                                            ...
                                            className={`${booking.reminderActive ? 'text-amber-500 hover:text-amber-600 dark:text-amber-400' : 'text-gray-400 dark:text-gray-400 hover:text-amber-500'} transition-colors p-0.5`}
                              ...
                              <div className="flex items-center justify-between">
                                <span className="truncate max-w-[120px] text-gray-400 dark:text-gray-400 italic">No bookings</span>
                                <button 
                                  onClick={() => onOpenBookings(item)}
